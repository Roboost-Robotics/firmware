...
void
MotorControllerManager::set_motor_speed(int motor_index, float desired_speed) {
...
void
MotorControllerManager::set_all_motor_speeds(float desired_speed) {
    for (auto &pair : motor_controllers_) {
...
float
MotorControllerManager::get_motor_speed(int motor_index) const {
    if (motor_index < 0 || motor_index >= motor_controllers_.size()) {
        // Handle invalid index error here...
...
int
MotorControllerManager::get_motor_count() const {
    return motor_controllers_.size();
}

void
MotorControllerManager::update() {
    for (auto &pair : motor_controllers_) {
