...
void OdometryPublisher::update(const Eigen::Vector3d& robot_velocity)
{
    ...
    Eigen::Vector3d pose =
        Eigen::Vector3d::Zero(); // Local pose variable if necessary

    double delta_theta = robot_velocity(2) * dt;
    ...
    pose(2) += delta_theta;
    pose(2) = atan2(sin(pose(2)), cos(pose(2))); // Normalize the angle

    odom_msg_.pose.pose.position.x = pose(0);
    odom_msg_.pose.pose.position.y = pose(1);
    odom_msg_.pose.pose.orientation = toQuaternion(0, 0, pose(2));
