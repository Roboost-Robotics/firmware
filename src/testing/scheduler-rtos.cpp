...
void periodicTask(void* pvParameters)
{
    ...
    while (true)
    {
        ...
        vTaskDelay(
            pdMS_TO_TICKS(200)); // Simulated task execution time of 200ms
        ...
        const TickType_t xMaxExecutionTime =
            pdMS_TO_TICKS(50); // Maximum allowed execution time of 50ms
...
void setup()
{
    ...
    // Create tasks
    // xTaskCreate(task1Function, "Task 1", 2048, (void*)"Task runs every 0.5
    // second", 5, NULL); xTaskCreate(task2Function, "Task 2", 2048,
    // (void*)"Task runs every 1 seconds", 5, NULL);
    xTaskCreate(periodicTask, "PeriodicTask", 2048, NULL, 1,
                NULL); // Adjust stack size and priority
}

void loop()
{
    // In FreeRTOS, the loop function should not contain any code and should
    // return.
}
