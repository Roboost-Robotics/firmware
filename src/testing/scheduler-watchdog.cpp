...
void taskFunction(void* parameters)
{
    ...
    while (true)
    {
        ...
        taskControls[taskIndex].lastCheckIn =
            xTaskGetTickCount(); // Update check-in time
...
void watchdogTask(void* parameters)
{
    while (true)
    {
        for (int i = 0; i < 2; i++)
        {
            ...
            if ((currentTime - taskControls[i].lastCheckIn) >
                taskControls[i].timeout)
            {
                Serial.println(
                    "Software Watchdog detected a problem with Task " +
                    String(i));
...
void setup()
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        xTaskCreate(taskFunction, "Task", 2048, &taskIndices[i], 5,
                    &taskControls[i].taskHandle);
